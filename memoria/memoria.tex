...
El propósito de este trabajo es la implementación de una serie de mejoras para este corrector que lo transformen a un modelo más sofisticado. Para ello, planteamos las siguientes mejoras:
...
\begin{enumerate}
    ...
    \item Para cada palabra de la frase
        \subitem Si esta en el vocabulario \rightarrow \ La dejamos como esta
        \subitem Si no está en el vocabulario \rightarrow \ Obtenemos todas las palabras cercanas y frecuentes junto con sus probabilidades de contexto (probabilidades que indican como de bien encajan esas palabras allí) y luego se elige la de mayor probabilidad.
...
\paragraph{Implementación.}
Para cumplir este objetivo, utilizamos un diccionario que asigna a cada letra su posición en el teclado QWERTY. Luego, al calcular la distancia euclídea de edición entre dos palabras, se modifica el coste de la operación de reemplazo/sustitución según la proximidad de las teclas involucradas. Por ejemplo, si la letra 'a' se sustituye por 's', que es una tecla adyacente, el coste de esta operación será menor que si se sustituyera por 'p', que está más alejada.

El algoritmo final para medir la distancia entre la frase corregida y la original pasa a ser la variante Damerau-Levenshtein, basada en Levenshtein, pero con los costes de edición ajustados según la proximidad de las teclas. Esto se implementa modificando la función que calcula el coste de cada operación de edición para incorporar la distancia física entre las teclas.
\paragraph{Hiperparámetros.}
...
robusto y preciso, aprendiendo las técnicas y métodos actuales para resolver
esta problemática. 
